...
# test_p0_pin_assignments.tcl and test_p0.sdc scripts. 
#
...
source [file join $script_dir test_p0_parameters.tcl]
load_package sdc_ext

proc test_p0_find_all_pins { mystring } {
...
proc test_p0_index_in_collection { col j } {
	set i 0
...
proc test_p0_get_clock_to_pin_name_mapping {} {
	set result [list]
	...
	foreach_in_collection clock $clocks_collection { 
		...
		set first_index [test_p0_index_in_collection $clock_target 0]
...
proc test_p0_get_clock_name_from_pin_name { pin_name } {
	set table [test_p0_get_clock_to_pin_name_mapping]
	foreach entry $table {
...
proc test_p0_get_clock_name_from_pin_name_vseries {pin_name suffix} {
	set name [test_p0_get_clock_name_from_pin_name $pin_name]
...
proc test_p0_get_clock_name_from_pin_name_pre_vseries {pin_name suffix} {
	set name [test_p0_get_clock_name_from_pin_name $pin_name]
	if {[string compare -nocase $name ""] == 0} {
		set pll_clock $pin_name
...
proc test_p0_get_or_add_clock_vseries_from_virtual_refclk {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-period "" /
	...
	set clock_name [test_p0_get_clock_name_from_pin_name $opts(-target)]
...
proc test_p0_get_or_add_clock_vseries {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-source "" /
...
proc test_p0_get_or_add_clock_pre_vseries {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-source "" /
...
proc test_p0_get_source_clock_pin_name {node_name} {

	set nodename ""
	set nodes [get_nodes $node_name]
	test_p0_traverse_fanin_up_to_depth [test_p0_index_in_collection $nodes 0] test_p0_is_node_type_pll_clk clock results_array 10
...
proc test_p0_find_all_keepers { mystring } {
...
proc test_p0_round_3dp { x } {
	return [expr { round($x * 1000) / 1000.0  } ]
}

proc test_p0_get_timequest_name {hier_name} {
	set sta_name ""
...
proc test_p0_are_entity_names_on { } {
...
proc test_p0_get_core_instance_list {corename} {
	set full_instance_list [test_p0_get_core_full_instance_list $corename]
	set instance_list [list]

	foreach inst $full_instance_list {
		set sta_name [test_p0_get_timequest_name $inst]
...
proc test_p0_get_core_full_instance_list {corename} {
...
proc test_p0_traverse_fanin_up_to_depth { node_id match_command edge_type results_array_name depth} {
	...
	for {set i 0} {$i != $number_of_fanin_edges} {incr i} {
		...
		if {$match_command == "" || [eval $match_command $fanin_id] != 0} {
			...
			test_p0_traverse_fanin_up_to_depth $fanin_id $match_command $edge_type results [expr {$depth - 1}]
		}
	}
}
proc test_p0_is_node_type_pll_inclk { node_id } {
...
proc test_p0_is_node_type_pin { node_id } {
...
proc test_p0_get_input_clk_id { pll_output_node_id } {
	if {[test_p0_is_node_type_pll_clk $pll_output_node_id]} {
		array set results_array [list]
		test_p0_traverse_fanin_up_to_depth $pll_output_node_id test_p0_is_node_type_pll_inclk clock results_array 2
		if {[array size results_array] == 1} {
			...
			test_p0_traverse_fanin_up_to_depth $pll_inclk_id test_p0_is_node_type_pin clock results_array 5
			if {[array size results_array] == 1} {
				# Fed by a dedicated input pin
				set pin_id [lindex [array names results_array] 0]
				set result $pin_id
			} else {
				test_p0_traverse_fanin_up_to_depth $pll_inclk_id test_p0_is_node_type_pll_clk clock pll_clk_results_array 1
				test_p0_traverse_fanin_up_to_depth $pll_inclk_id test_p0_is_node_type_pll_clk clock pll_clk_results_array2 2
				if {[array size pll_clk_results_array] == 1} {
					...
					set result [test_p0_get_input_clk_id $source_pll_clk_id]
					if {$result != -1} {
					...
					test_p0_traverse_fanin_up_to_depth $pll_inclk_id test_p0_is_node_type_pin clock results_array 20
					if {[array size results_array] == 1} {
						set pin_id [lindex [array names results_array] 0]
						set result $pin_id
					} else {
		...
		error "Internal error: test_p0_get_input_clk_id only works on PLL output clocks"
	}
	return $result
}

proc test_p0_is_node_type_pll_clk { node_id } {
	set cell_id [get_node_info -cell $node_id]
...
proc test_p0_get_pll_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
	...
	foreach node_id $dest_id_list {
		test_p0_traverse_fanin_up_to_depth $node_id test_p0_is_node_type_pll_clk clock clk_array $search_depth
	}
...
proc test_p0_get_pll_clock_name { clock_id } {
	set clock_name [get_node_info -name $clock_id]

	return $clock_name
}

proc test_p0_get_pll_clock_name_for_acf { clock_id pll_output_wire_name } {
	set clock_name [get_node_info -name $clock_id]
...
proc test_p0_get_output_clock_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	...
	test_p0_get_pll_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}

proc test_p0_get_output_clock_id2 { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
...
proc test_p0_is_node_type_clkbuf { node_id } {
	set cell_id [get_node_info -cell $node_id]
	if {$cell_id == ""} {
		set result 0
	} else {
...
proc test_p0_get_clkbuf_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
		upvar 1 $clock_id_name clock_id
	}
	set clock_id -1
	...
	array set clk_array [list]
	foreach node_id $dest_id_list {
		test_p0_traverse_fanin_up_to_depth $node_id test_p0_is_node_type_clkbuf clock clk_array $search_depth
	}
	if {[array size clk_array] == 1} {
		set clock_id [lindex [array names clk_array] 0]
		set clk [get_node_info -name $clock_id]
...
proc test_p0_get_output_clock_clkbuf_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
	...
	test_p0_get_clkbuf_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}


proc test_p0_is_node_type_clk_phase_select { node_id } {
	set cell_id [get_node_info -cell $node_id]
	if {$cell_id == ""} {
		set result 0
	} else {
...
proc test_p0_get_clk_phase_select_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
		upvar 1 $clock_id_name clock_id
	}
	set clock_id -1
	...
	array set clk_array [list]
	foreach node_id $dest_id_list {
		test_p0_traverse_fanin_up_to_depth $node_id test_p0_is_node_type_clk_phase_select clock clk_array $search_depth
	}
	if {[array size clk_array] == 1} {
		set clock_id [lindex [array names clk_array] 0]
		set clk [get_node_info -name $clock_id]
...
proc test_p0_get_output_clock_clk_phase_select_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
	...
	test_p0_get_clk_phase_select_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}
...
proc test_p0_get_names_in_collection { col } {
	set res [list]
...
proc test_p0_static_map_expand_list { FH listname pinname } {
...
proc test_p0_static_map_expand_list_of_list { FH listname pinname } {
	upvar $listname local_list
...
proc test_p0_static_map_expand_string { FH stringname pinname } {
...
proc test_p0_format_3dp { x } {
	return [format %.3f $x]
}

proc test_p0_get_colours { x y } {
...
proc test_p0_max_in_collection { col attribute } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {$i == 0} {
...
proc test_p0_min_in_collection { col attribute } {
	set i 0
	set min 0
	foreach_in_collection path $col {
		if {$i == 0} {
...
proc test_p0_min_in_collection_to_name { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc test_p0_min_in_collection_from_name { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc test_p0_max_in_collection_to_name { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[get_node_info -name [get_path_info $path -to]] == $name} {
...
proc test_p0_max_in_collection_from_name { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[get_node_info -name [get_path_info $path -from]] == $name} {
...
proc test_p0_min_in_collection_to_name2 { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc test_p0_min_in_collection_from_name2 { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc test_p0_max_in_collection_to_name2 { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[regexp $name [get_node_info -name [get_path_info $path -to]]]} {
...
proc test_p0_max_in_collection_from_name2 { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[regexp $name [get_node_info -name [get_path_info $path -from]]]} {
...
proc test_p0_get_max_clock_path_delay_through_clock_node {from through to} {
	set init 0
	set max_delay 0
...
proc test_p0_get_min_clock_path_delay_through_clock_node {from through to} {
	set init 0
	set min_delay 0
...
proc test_p0_get_min_aiot_delay {pinname} {
...
proc test_p0_get_rise_aiot_delay {pinname} {
...
proc test_p0_get_fall_aiot_delay {pinname} {
...
proc test_p0_get_aiot_attr {pinname attr} {
...
proc test_p0_get_pll_phase_shift {output_counter_name} {
	load_package atoms
	read_atom_netlist
	set phase_shift ""
...
# ----------------------------------------------------------------
#
proc test_p0_get_io_standard {target_pin} {
#
...
# ----------------------------------------------------------------
	...
	set io_std [test_p0_get_fitter_report_pin_info $target_pin "I/O Standard" -1]
	if {$io_std == ""} {
		return "UNKNOWN"
	}
	set result ""
...
proc test_p0_get_rzq_pins { instname all_rzq_pins } {
	upvar $all_rzq_pins rzqpins
	load_package atoms
	read_atom_netlist
	set rzq_pins [ list ]
	set entity_names_on [ test_p0_are_entity_names_on ]
...
proc test_p0_get_acv_read_offset { period dqs_phase dqs_period } {
...
proc test_p0_traverse_atom_path {atom_id atom_oport_id path} {
		...
		if {[get_atom_node_info -key type -node $atom_id] == $atom_type} {
			if {$next_direction == "end"} {
				...
				if {$atom_iport != -1} {
					...
					set result [test_p0_traverse_atom_path $source_atom $source_oterm [lrange $path 1 end]]
				} elseif {$atom_optional == "-optional"} {
					set result [test_p0_traverse_atom_path $atom_id $atom_oport_id [lrange $path 1 end]]
				}
				...
				if {$atom_oport != -1} {
					...
					foreach dest $oport_fanout {
						...
						set fanout_result_list [test_p0_traverse_atom_path $dest_atom -1 [lrange $path 1 end]]
...
proc test_p0_traverse_to_ddio_out_pll_clock {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		...
		set pll_id_list [test_p0_traverse_atom_path $pin_id -1 $pin_to_pll_path]
...
proc test_p0_traverse_to_leveling_delay_chain {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		...
		set leveling_id_list [test_p0_traverse_atom_path $pin_id -1 $pin_to_leveling_path]
...
proc test_p0_traverse_to_clock_phase_select {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		...
		set cps_id_list [test_p0_traverse_atom_path $pin_id -1 $pin_to_cps_path]
...
proc test_p0_traverse_to_clkbuf {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		...
		set clkbuf_id_list [test_p0_traverse_atom_path $pin_id -1 $pin_to_clkbuf_path]
...
proc test_p0_traverse_to_dll {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	...
	set dll_id_list [test_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_dll_path]
	set result ""
...
proc test_p0_check_hybrid_interface { inst pins_array_name mem_if_memtype } {
	...
	foreach dq_dm_pin $all_dq_dm_pins {
		set io_type [test_p0_get_fitter_report_pin_io_type_info $dq_dm_pin]
...
proc test_p0_verify_flexible_timing_assumptions { inst pins_array_name mem_if_memtype } {
	return 1
}

proc test_p0_verify_high_performance_timing_assumptions { inst pins_array_name mem_if_memtype } {
	upvar $pins_array_name pins
	...
	if {$num_errors == 0} {
		set msg_list [list]
		set dll_name [test_p0_traverse_to_dll $dqs msg_list]
		set clk_to_write_d [test_p0_traverse_to_ddio_out_pll_clock [lindex $all_d_list 0] msg_list]
		set clk_to_write_clock [test_p0_traverse_to_ddio_out_pll_clock [lindex $all_write_dqs_list 0] msg_list]
		set clk_to_ck_ckn [test_p0_traverse_to_ddio_out_pll_clock [lindex $pins(ck_pins) 0] msg_list]
...
proc test_p0_get_tccs { mem_if_memtype dqs_list period args} {
	...
	set interface_type [test_p0_get_io_interface_type $dqs_list]
	# The tCCS for a VHPAD interface is the same as a HPAD interface
	if {$interface_type == "VHPAD"} {
		set interface_type "HPAD"
	}
	set io_std [test_p0_get_io_standard [lindex $dqs_list 0]]
  	set result [list 0 0]
	if {$interface_type != "" && $interface_type != "UNKNOWN" && $io_std != "" && $io_std != "UNKNOWN"} {
		...
		if {$options(-ddr3_discrete) == 0 && $options(-write_deskew) != "none"} {
			set mode [test_p0_get_deskew_freq_range $tccs_params $period]
			if {$mode == [list]} {
...
proc test_p0_get_deskew_freq_range {timing_params period} {
	set mode [list]
...
proc test_p0_get_tsw { mem_if_memtype dqs_list period args} {
	global TimeQuestInfo
...
# ----------------------------------------------------------------
#
proc test_p0_get_fitter_report_pin_info_from_report {target_pin info_type pin_report_id} {
#
# Description: Gets the report field for the given pin in the given report
#
# ----------------------------------------------------------------
	set pin_name_column [test_p0_get_report_column $pin_report_id "Name"]
	set info_column [test_p0_get_report_column $pin_report_id $info_type]
	set result ""
...
# ----------------------------------------------------------------
#
proc test_p0_get_fitter_report_pin_info {target_pin info_type preferred_report_id {found_report_id_name ""}} {
#
...
# ----------------------------------------------------------------
	...
	if {$preferred_report_id == -1} {
		...
		for {set pin_report_index 0} {$pin_report_index != [llength $pin_report_list] && $result == ""} {incr pin_report_index} {
			...
			if {$pin_report_id != -1} {
				set result [test_p0_get_fitter_report_pin_info_from_report $target_pin $info_type $pin_report_id]
				if {$result != ""} {
					set found_report_id $pin_report_id
				}
			} else {
				post_message -type error "test_p0_pin_map.tcl: Failed to find fitter report. If report timing is run after an ECO, the user must set_global_assignment -name ECO_REGENERATE_REPORT ON in test_p0.qsf and in test_p0_pin_assignment.tcl files and rerun ECO and STA"
			}
		}
	} else {
		set result [test_p0_get_fitter_report_pin_info_from_report $target_pin $info_type $preferred_report_id]
		if {$result != ""} {
...
# ----------------------------------------------------------------
#
proc test_p0_get_fitter_report_pin_io_type_info {target_pin} {
#
...
# ----------------------------------------------------------------
	...
	if {$pin_report_id != -1} {
		set pin_name_column [test_p0_get_report_column $pin_report_id "Pin Name/Usage"]
		set info_column [test_p0_get_report_column $pin_report_id "I/O Type"]
		if {$pin_name_column == 0 && 0} {
			set row_index [get_report_panel_row_index -id $pin_report_id $target_pin]
			if {$row_index != -1} {
				set row [get_report_panel_row -id $pin_report_id -row $row_index]
		...
		if {$pin_report_id != -1} {
			...
			set report_rows [get_number_of_rows -id $pin_report_id]
			set pin_name_column [test_p0_get_report_column $pin_report_id "Name"]
			set info_column [test_p0_get_report_column $pin_report_id "I/O Edge"]
...
# ----------------------------------------------------------------
#
proc test_p0_get_io_interface_type {pin_list} {
#
...
# ----------------------------------------------------------------
	...
	foreach target_pin $pin_list {
		set io_bank [test_p0_get_fitter_report_pin_info $target_pin "I/O Bank" $preferred_report_id preferred_report_id]
...
# ----------------------------------------------------------------
#
proc test_p0_get_report_column { report_id str} {
#
...
proc test_p0_traverse_to_dll_id {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set dqs_pin_id [get_atom_node_by_name -name $dqs_pin]
	set dqs_to_dll_path [list {IO_PAD fanout PADOUT} {IO_IBUF fanout O} {DQS_DELAY_CHAIN fanin DELAYCTRLIN} {DLL end DELAYCTRLOUT}]
	set dll_id_list [test_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_dll_path]
...
proc test_p0_traverse_to_dqs_delaychain_id {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set dqs_pin_id [get_atom_node_by_name -name $dqs_pin]
	set dqs_to_delaychain_path [list {IO_PAD fanout PADOUT} {IO_IBUF fanout O} {DQS_DELAY_CHAIN atom}]
	set delaychain_id_list [test_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_delaychain_path]
	set delaychain_id -1
...
proc test_p0_get_dqs_phase { dqs_pins } {
	set dqs_phase -100
	set dqs0 [lindex $dqs_pins 0]
	if {$dqs0 != ""} {
		set dqs_delay_chain_id [test_p0_traverse_to_dqs_delaychain_id $dqs0 msg_list]
...
proc test_p0_get_dqs_period { dqs_pins } {
	set dqs_period -100
	set dqs0 [lindex $dqs_pins 0]
	if {$dqs0 != ""} {
		set dll_id [test_p0_traverse_to_dll_id $dqs0 msg_list]
		if {$dll_id != -1} {
...
proc test_p0_get_operating_conditions_number {} {
...
proc test_p0_get_ddr_pins { instname allpins } {
	# We need to make a local copy of the allpins associative array
	upvar allpins pins
	global ::GLOBAL_test_p0_dqs_group_size
	global ::GLOBAL_test_p0_number_of_dqs_groups
	...
	for { set i 0 } { $i < $::GLOBAL_test_p0_number_of_dqs_groups } { incr i } {
		...
		set q_group [ list ]
		for { set j 0 } { $j < $::GLOBAL_test_p0_dqs_group_size } { incr j } { 
			set index [ expr $i * $::GLOBAL_test_p0_dqs_group_size + $j ]
			set q_string ${instname}|p0|umemphy|uio_pads|dq_ddio\[$i\].ubidir_dq_dqs|${dqs_inst}pad_gen\[${j}\].data_out|o
			set tmp_q_pins [ test_p0_get_names_in_collection [ get_fanouts $q_string ] ]
		...
		if { [llength $q_group] != $::GLOBAL_test_p0_dqs_group_size} { 
			post_sdc_message critical_warning "Could not find correct number of D pins for K pin $i. \
				Found [llength $q_group] pins. Expecting ${::GLOBAL_test_p0_dqs_group_size}." 
		}
	...
	foreach {pin_type pattern} $patterns {
		set local_pins [ test_p0_get_names_in_collection [ get_fanouts $pattern ] ]
	...
	# CLOCK OUTPUT PLL
	set pll_ck_clock_id [test_p0_get_output_clock_id $pins(ck_pins) "CK Output" msg_list]
	if {$pll_ck_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(ck_pins)]"
	} else {
		set pll_ck_clock [test_p0_get_pll_clock_name $pll_ck_clock_id]
	}
	...
	# AFI CLOCK PLL
	set pll_afi_clock_id [test_p0_get_output_clock_id $pins(afi_ck_pins) "AFI CK" msg_list]
	if {$pll_afi_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_ck_pins)]"
	} else {
		set pll_afi_clock [test_p0_get_pll_clock_name $pll_afi_clock_id]
	}
	set pins(pll_afi_clock) $pll_afi_clock

	# AFI PHY CLOCK PLL
	set pll_afi_phy_clock_id [test_p0_get_output_clock_id $pins(afi_phy_ck_pins) "AFI PHY CK" msg_list]
	if {$pll_afi_phy_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_phy_ck_pins)]"
	} else {
		set pll_afi_phy_clock [test_p0_get_pll_clock_name $pll_afi_phy_clock_id]
	}
	set pins(pll_afi_phy_clock) $pll_afi_phy_clock

	# DQ PLL
	set pll_dq_write_clock_id [test_p0_get_output_clock_id [ join [ join $pins(q_groups) ]] "Write DQ CK" msg_list 20]
	if {$pll_dq_write_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [ join [ join $pins(q_groups) ]]"
	} else {
		set pll_dq_write_clock [test_p0_get_pll_clock_name $pll_dq_write_clock_id]
	}
	set pins(pll_dq_write_clock) $pll_dq_write_clock
	
	# DQS PLL
	set pll_write_clock_id [test_p0_get_output_clock_id $pins(dqs_pins) "Write CK" msg_list 20]
	if {$pll_write_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins $pins(dqs_pins)"
	} else {
		set pll_write_clock [test_p0_get_pll_clock_name $pll_write_clock_id]
	}
	set pins(pll_write_clock) $pll_write_clock	

	# AC PLL
	set pll_ac_clock_id [test_p0_get_output_clock_id $pins(add_pins) "Address/Command Output" msg_list]
	if {$pll_ac_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(add_pins)]"
	} else {
		set pll_ac_clock [test_p0_get_pll_clock_name $pll_ac_clock_id]
	}
	set pins(pll_ac_clock) $pll_ac_clock


	set pll_avl_clock_id [test_p0_get_output_clock_id $pins(avl_ck_pins) "Avalon Bus CK" msg_list]
	if {$pll_avl_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_ck_pins)]"
	} else {
		set pll_avl_clock [test_p0_get_pll_clock_name $pll_avl_clock_id]
	}
	set pins(pll_avl_clock) $pll_avl_clock


	set pll_config_clock_id [test_p0_get_output_clock_id $pins(config_ck_pins) "Config CK" msg_list]
	if {$pll_config_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "test_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "test_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(config_ck_pins)]"
	} else {
		set pll_config_clock [test_p0_get_pll_clock_name $pll_config_clock_id]
	}
	set pins(pll_config_clock) $pll_config_clock


	# REF CLOCK
	set pll_ref_clock_id [test_p0_get_input_clk_id $pll_ck_clock_id]
	if {$pll_ref_clock_id == -1} {
		post_message -type error "test_p0_pin_map.tcl: Failed to find PLL reference clock"
	} else {
...
proc test_p0_initialize_ddr_db { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	global ::GLOBAL_test_p0_corename

	post_sdc_message info "Initializing DDR database for CORE $::GLOBAL_test_p0_corename"
	set instance_list [test_p0_get_core_instance_list $::GLOBAL_test_p0_corename]
	# set local_ddr_db(instance_list) $instance_list

	foreach instname $instance_list {
		post_sdc_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_test_p0_corename INSTANCE: $instname"

		test_p0_get_ddr_pins $instname allpins

		test_p0_verify_ddr_pins allpins

		set local_ddr_db($instname) [ array get allpins ]
	}
}

proc test_p0_verify_ddr_pins { pins_par } {
	upvar $pins_par pins
	...
	if { $::GLOBAL_test_p0_number_of_dm_pins != $counted_dm_pins } {
		post_message -type critical_warning "Unexpected number of detected DM pins: $counted_dm_pins"
		post_message -type critical_warning "   expected: $::GLOBAL_test_p0_number_of_dm_pins"
	}
...
proc test_p0_get_all_instances_div_names { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc test_p0_get_all_instances_dqs_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc test_p0_dump_all_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	set instnames [ array names local_ddr_db ]

	set filename "${::GLOBAL_test_p0_corename}_all_pins.txt"
	...
	puts $FH "# PIN MAP for core < $::GLOBAL_test_p0_corename >"
	puts $FH "#"
	puts $FH "# Generated by ${::GLOBAL_test_p0_corename}_pin_assignments.tcl"
	puts $FH "#"
...
proc test_p0_dump_static_pin_map { ddr_db_par filename } {
	upvar $ddr_db_par local_ddr_db
	...
	puts $FH "# AUTO-GENERATED static pin map for core < $::GLOBAL_test_p0_corename >"
	puts $FH ""
	puts $FH "proc ${::GLOBAL_test_p0_corename}_initialize_static_ddr_db { ddr_db_par } {"
	...
	foreach instname $instnames {
		...
		test_p0_static_map_expand_list $FH pins dqs_pins
		test_p0_static_map_expand_list $FH pins dqsn_pins

		test_p0_static_map_expand_list_of_list $FH pins q_groups

		puts $FH ""
		puts $FH "   set pins(all_dq_pins) \[ join \[ join \$pins(q_groups) \] \]"

		test_p0_static_map_expand_list $FH pins dm_pins

		test_p0_static_map_expand_list $FH pins ck_pins
		test_p0_static_map_expand_list $FH pins ckn_pins

		test_p0_static_map_expand_list $FH pins add_pins
		test_p0_static_map_expand_list $FH pins cmd_pins
		test_p0_static_map_expand_list $FH pins reset_pins
		test_p0_static_map_expand_list $FH pins ba_pins

		puts $FH ""
		puts $FH "   set pins(ac_pins) \[ concat \$pins(add_pins) \$pins(ba_pins) \$pins(cmd_pins) \$pins(reset_pins)\]"

		test_p0_static_map_expand_string $FH pins pll_ref_clock
		test_p0_static_map_expand_string $FH pins pll_afi_clock
		test_p0_static_map_expand_string $FH pins pll_afi_phy_clock
		test_p0_static_map_expand_string $FH pins pll_ck_clock
		test_p0_static_map_expand_string $FH pins pll_dq_write_clock
		test_p0_static_map_expand_string $FH pins pll_write_clock
		test_p0_static_map_expand_string $FH pins pll_ac_clock
		test_p0_static_map_expand_string $FH pins pll_avl_clock
		test_p0_static_map_expand_string $FH pins pll_config_clock
		...
		test_p0_static_map_expand_string $FH pins read_capture_ddio
		test_p0_static_map_expand_string $FH pins afi_reset_reg
		test_p0_static_map_expand_string $FH pins seq_reset_reg
		test_p0_static_map_expand_string $FH pins sync_reg
		test_p0_static_map_expand_string $FH pins fifo_wraddress_reg 
		test_p0_static_map_expand_string $FH pins fifo_wrdata_reg 
		test_p0_static_map_expand_string $FH pins fifo_rddata_reg
