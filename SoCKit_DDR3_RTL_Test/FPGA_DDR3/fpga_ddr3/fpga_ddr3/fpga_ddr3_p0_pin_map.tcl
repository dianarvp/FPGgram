		...
		if {[array size results_array] == 1} {
			...
			if {[array size results_array] == 1} {
				...
				if {[array size pll_clk_results_array] == 1} {
					...
					fpga_ddr3_p0_traverse_fanin_up_to_depth $pll_inclk_id fpga_ddr3_p0_is_node_type_pin clock results_array 20
					if {[array size results_array] == 1} {
						set pin_id [lindex [array names results_array] 0]
						set result $pin_id
					} else {
...
proc fpga_ddr3_p0_get_acv_read_offset { period dqs_phase dqs_period } {
...
proc fpga_ddr3_p0_get_dqs_period { dqs_pins } {
	set dqs_period -100
	set dqs0 [lindex $dqs_pins 0]
	if {$dqs0 != ""} {
		set dll_id [fpga_ddr3_p0_traverse_to_dll_id $dqs0 msg_list]
		if {$dll_id != -1} {
...
proc fpga_ddr3_p0_get_ddr_pins { instname allpins } {
	...
	if {[string compare -nocase $pins(driver_core_ck_pins) ""] != 0 && [get_collection_size [get_registers -nowarn $pins(driver_core_ck_pins)]] > 0} {
		set pll_driver_core_clock_id [fpga_ddr3_p0_get_output_clock_id $pins(driver_core_ck_pins) "DRIVER CORE CK" msg_list]
		if {$pll_driver_core_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "fpga_ddr3_p0_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "fpga_ddr3_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(driver_core_ck_pins)]"
		} else {
			set pll_driver_core_clock [fpga_ddr3_p0_get_pll_clock_name $pll_driver_core_clock_id]
		}
	}
	...
	# AFI half clock
	set pll_afi_half_clock_id [fpga_ddr3_p0_get_output_clock_id $pins(afi_half_ck_pins) "AFI HALF CK" msg_list]
	if {$pll_afi_half_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "fpga_ddr3_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "fpga_ddr3_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_half_ck_pins)]"
	} else {
		set pll_afi_half_clock [fpga_ddr3_p0_get_pll_clock_name $pll_afi_half_clock_id]
	}
	...
	if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
		set pll_avl_phy_clock_id [fpga_ddr3_p0_get_output_clock_id $pins(avl_phy_ck_pins) "Avalon PHY Bus CK" msg_list]
		if {$pll_avl_phy_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "fpga_ddr3_p0_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "fpga_ddr3_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_phy_ck_pins)]"
		} else {
			set pll_avl_phy_clock [fpga_ddr3_p0_get_pll_clock_name $pll_avl_phy_clock_id]
		}
	}
	...
	if {$pll_ref_clock_id == -1} {
		post_message -type error "fpga_ddr3_p0_pin_map.tcl: Failed to find PLL reference clock"
	} else {
...
proc fpga_ddr3_p0_dump_static_pin_map { ddr_db_par filename } {
	...
	foreach instname $instnames {
		...
		fpga_ddr3_p0_static_map_expand_string $FH pins pll_afi_half_clock
		fpga_ddr3_p0_static_map_expand_string $FH pins pll_avl_clock
		fpga_ddr3_p0_static_map_expand_string $FH pins pll_avl_phy_clock
		fpga_ddr3_p0_static_map_expand_string $FH pins pll_config_clock
		fpga_ddr3_p0_static_map_expand_string $FH pins pll_driver_core_clock
